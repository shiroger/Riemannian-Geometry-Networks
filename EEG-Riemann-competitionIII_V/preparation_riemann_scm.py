...
scm_path = 'data/scm'
riemann_scm_norm_path = 'data/scm_riemann_norm'

riemann_mean_path = 'data/riemann_mean.mat'
inp_stats_file = "data/input_norm_states_scm_riemann.norm"
...
if cal_riemann_mean:
    ...
    riemann_mean = utils.riemann_mean_by_classes(all_scm, all_labels)
...
if cal_normalization_coef:
    ...
    hand_idx = np.where((all_labels == 2) | (all_labels == 3))
    scm_hand = combined_matrix[hand_idx[1]]
    tangent_left = utils.tangent_space_mapping_2D_f32(scm_hand, riemann_mean[0])
    tangent_right = utils.tangent_space_mapping_2D_f32(scm_hand, riemann_mean[1])
    ...
    norm_matrix = np.hstack((norm_matrix_left, norm_matrix_right, norm_matrix_scm))
...
if norm_file_save:
    ...
    left_scaler_scale = norm_matrix[3]
    right_scaler_mean = norm_matrix[1]
    right_scaler_scale = norm_matrix[4]
    scm_scaler_mean = norm_matrix[2]
    scm_scaler_scale = norm_matrix[5]
    ...
    for file in files:
        ...
        norm_riemann_scm[:, 2, :, :] = (norm_riemann_scm[:, 2, :, :] - scm_scaler_mean) / scm_scaler_scale
