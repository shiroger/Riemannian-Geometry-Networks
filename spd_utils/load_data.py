...
def get_data(subject,training,PATH):
    ...
    if training:
        ...
        # a = mne.io.read_raw_gdf(PATH+'A0'+str(subject)+'T.gdf')
    else:
        a = sio.loadmat(PATH+'\A0'+str(subject)+'E.mat')
        # a = mne.io.read_raw_gdf(PATH + 'A0' + str(subject) + 'E.gdf')
    a_data = a['data']

    for ii in range(0,a_data.size):
        a_data1     = a_data[0,ii]
        a_data2     = [a_data1[0,0]]    # = [a_data1]
        a_data3     = a_data2[0]
        a_X         = a_data3[0]
        a_trial     = a_data3[1]
        a_y         = a_data3[2]
...
class load_BCIC:
    def __init__(self, sub, TorE = True, alg_name ='Tensor_CSPNet', session_no = 1, scenario = 'CV'):
        ...
        elif scenario == 'CV_all':
            self.x_1, self.y_labels_1 = get_data(sub, True, 'dataset/')     # T
            self.x_2, self.y_labels_2 = get_data(sub, False, 'dataset/')    # E
            self.y_labels = np.concatenate((self.y_labels_1, self.y_labels_2))
            fbank = FilterBank(fs=250, pass_width=self.freq_seg)
            _ = fbank.get_filter_coeff()

            x_1_fb = fbank.filter_data(self.x_1, window_details={'tmin':3.0, 'tmax':7.0}).transpose(1, 0, 2, 3)
            x_2_fb  = fbank.filter_data(self.x_2, window_details={'tmin':3.0, 'tmax':7.0}).transpose(1, 0, 2, 3)

            x_fb = np.concatenate((x_1_fb, x_2_fb), axis=0)

            self.x_stack = self._tensor_stack(x_fb)

            train_indices = np.load(
                'index/BCIC_index/sess' + str(session_no) + '_sub' + str(sub) + '_train_index.npy', allow_pickle=True)
            test_indices = np.load(
                'index/BCIC_index/sess' + str(session_no) + '_sub' + str(sub) + '_test_index.npy', allow_pickle=True)

            # train_indices_reshaped = np.expand_dims(train_indices, axis=1)
            # test_indices_reshaped = np.expand_dims(test_indices, axis=1)
            train_indices_all = []
            test_indices_all = []
            for elem in train_indices:
                new_elem = np.concatenate([elem, elem])
                train_indices_all.append(new_elem)

            for elem in test_indices:
                new_elem = np.concatenate([elem, elem])
                test_indices_all.append(new_elem)

            self.train_indices = train_indices_all
            self.test_indices = test_indices_all
            print('ok')
