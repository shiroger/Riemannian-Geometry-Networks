'''
#####################################################################################################################
Discription: EEG input, y*yT as Riemannian Block
#######################################################################################################################
'''
import glob
...
# model_name = f'weight/subject/model_classification_a
# ll_riemann_middle_filter_{subject_id}.pth'
...
for p in range(events_train.shape[0]):
    all_scm.append(eeg_train[:, events_train[p, 0]:events_train[p, 0] + 4*fs])
    all_labels.append(events_train[p, 2])
for p in range(events_test.shape[0]):
    all_scm.append(eeg_test[:, events_test[p, 0]:events_test[p, 0] + 4*fs])
    all_labels.append(events_test[p, 2])
all_scm = np.stack(all_scm)
all_labels = np.stack(all_labels)
all_scm = all_scm.transpose(1, 2, 0)

if normalize_EEG:
    raw_shape = all_scm.shape  # (44,1000,trails)
    all_scm = np.reshape(all_scm, (raw_shape[0], -1))  # (44,1000,trails)->(44,1000*trails)
    all_scm = all_scm - np.mean(all_scm, 0)
    all_scm = np.reshape(all_scm, raw_shape)  # (44,1000*trails)->(44,1000,trails)
    for i in range(inp_dim):
        for j in range(all_scm.shape[1]):
            mean_x = np.mean(all_scm[i, j, :])
            std_x = np.std(all_scm[i, j, :])
            all_scm[i, j, :] = (all_scm[i, j, :] - mean_x) / std_x
if timelap:
    ...
    num_raw_samples = all_scm.shape[-1]
    all_scm_lap = np.empty((inp_dim, len_new_samples, num_raw_samples * num_lap))
    for t in range(num_lap):
        all_scm_lap[:, :, t * num_raw_samples:(t + 1) * num_raw_samples] = all_scm[:,
                                                                            t * interval:t * interval + len_new_samples,
                                                                            :]
    all_scm = all_scm_lap
    all_labels = np.tile(all_labels, num_lap)
if time_split:
    ...
    all_scm_lap = np.empty((inp_dim, len_new_samples, num_raw_samples * num_lap))
    for t in range(num_lap):
        all_scm_lap[:, :, t * num_raw_samples:(t + 1) * num_raw_samples] = all_scm[:, t * interval:t * interval + len_new_samples, :]
    all_scm = all_scm_lap
    all_labels = np.tile(all_labels, num_lap)

all_scm = all_scm.transpose((2, 0, 1))  # (44,1000,trails)->(trails,44,1000)


# all_scm = np.concatenate(all_scm, axis=0)   # (84,64-channel,64-samples)
# all_labels = np.concatenate(all_labels, axis=1)     # (1,84)
