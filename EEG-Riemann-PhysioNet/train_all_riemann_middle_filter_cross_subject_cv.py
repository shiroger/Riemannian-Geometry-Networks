...
inp_stats_file = f"data/20-Subjects/input_norm_states_scm_filter_cross.norm"
riemann_mean_norm_path = f'data/20-Subjects/all_riemann_mean_filter_norm_cross.mat'
...
all_scm = np.concatenate(all_scm, axis=0)   # (84,64-channel,320-samples)
all_labels = np.concatenate(all_labels, axis=1)     # (1,84)

all_labels_onehot = load_onehot_label(all_labels+1)
...
for fold, (train_index, test_index) in enumerate(kf.split(all_scm, all_labels_onehot)):
    ...
    model_name = f'weight/subject_cv_5/model_classification_all_riemann_middle_filter_cross_{fold}.pth'
