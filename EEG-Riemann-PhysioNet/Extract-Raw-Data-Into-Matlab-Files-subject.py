...
def get_electrode_positions():
    ...
    """
    positions = dict()
    with io.open("data\eeg-motor-movementimagery-dataset-1.0.0/files/electrode_positions.txt", "r") as pos_file:
...
def load_physionet_data(subject_id, num_classes=4, long_edge=False):
    ...
    base_path = 'data/eeg-motor-movementimagery-dataset-1.0.0/files/S%03d/S%03dR%02d.edf'
...
SAVE = 'data/20-Subjects'
if not os.path.exists(SAVE):
    os.mkdir(SAVE)

subject = range(1, 21)
for subject_id in subject:
    t, l, loc, fs = load_physionet_data(subject_id, nclasses, long_edge=False)  # t(84,640,64)  l(84,4)
    l = np.argmax(l, axis=1)
    if nclasses == 2 and t.shape[0] != 42:
        # drop subjects with less trials
        continue
    sio.savemat(SAVE + '/Dataset_%d.mat' % int(subject_id), {'EEG': t, 'labels': l})
    # sio.savemat(SAVE + '/Labels_%d.mat' % int(subject_id), {'Labels': l})
print("Finished saving %d subjects" % int(subject_id))
# # Save 64 electrodes
# for i in range(0, 64):
#     electrodes = [i]
#     X = 'X_' + str(i)
